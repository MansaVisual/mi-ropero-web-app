  ...
  const busquedaPrimera = () => {
    ...
    if (putCategory !== "") {
      ...
      apiFetch(col, "tiendas", "detail").then((res) => {
        if (res.status === "success") {
          setFiltrosCategoria(
            res.result.search_productos_categorias[0].hijas[0].caracteristicas
          );
          setProds(res.result);
          setTotalPages(Number(res.result.search_productos_total_paginas));
        }
  ...
  return (
    <Container maxWidth="xl">
      <Grid
        ...
        sx={{
          px:
            isMobile || isMobileBigScreen ? "16px" : isTablet ? "20px" : "74px",
          py: "40px",
        }}
      >
        ...
        <Grid item xs={12} sm={6} md={9}>
          {buscandoRoperos ? (
          ...
          ) : prods.search_productos !== undefined &&
            ...
            >
              <Box sx={{ mr: "20px" }}>
                <img src={lupa} width={40} height={40} alt="not found icon" />
              </Box>
              <Box>
                <Typography
                  sx={{
