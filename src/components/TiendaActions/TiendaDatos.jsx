...
const TiendaDatos = () => {
  ...
  const { tiendaData, tiendaDetail, dataUpdate } =
  ...
  const pausarTienda = (type) => {
    Swal.fire({
      ...
      text:
        type !== "5"
          ? "Pausarás tu tienda hasta que lo desees"
          : "Despausarás tu tienda",
  ...
  const reporteSem = (type) => {
    Swal.fire({
      title:
        type === "1" ? "DESACTIVAR REPORTE SEMANAL" : "ACTIVAR REPORTE SEMANAL",
      text:
        type === "1"
          ? "Dejará de recibir un reporte semanal con las estadísticas de tu tienda"
          : "Recibirá un reporte semanal con las estadísticas de tu tienda",
      iconHtml: `<img src=${logo} alt="LOGO">`,
      showCloseButton: true,
      confirmButtonText: "CONTINUAR",
      customClass: {
      ...
      if (res.isConfirmed) {
        ...
        pause.append("idtienda", tiendaDetail.idtienda);
        if (type === "1") {
          pause.append("recibe_reporte", 0);
        } else {
          pause.append("recibe_reporte", 1);
        }
